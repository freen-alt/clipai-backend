...
app.post('/api/clip', async (req, res) => {
  ...
  try {
    await execAsync(
      `yt-dlp -f "bestvideo[height<=480][ext=mp4]+bestaudio[ext=m4a]/best[height<=480][ext=mp4]/best" --merge-output-format mp4 -o "${rawPath}" --no-playlist "${url}"`,
  ...
  if (!fs.existsSync(rawPath)) {
    fs.rmSync(jobDir, { recursive: true, force: true });
    return res.status(500).json({ error: 'Vídeo não foi baixado corretamente.' });
  }
  ...
  for (const clip of clips) {
    ...
    try {
      await execAsync(
        `ffmpeg -y -loglevel error -ss ${clip.startTime} -i "${rawPath}" -t ${duration} -c:v libx264 -preset veryfast -crf 26 -vf "scale='min(854,iw)':-2" -c:a aac -b:a 96k "${outPath}"`,
